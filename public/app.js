...
return '<div class="card"><div class="card-top"><span class="card-code">'+c(v.venda_code||"")+'</span><span class="card-prod">'+c(v.modelo_rotulo||"")+(v.capacidade?" "+c(v.capacidade):"")+(v.cor?" "+c(v.cor):"")+'</span></div><div class="card-sub">'+c(v.cliente_nome||"sem cliente")+(v.data_venda?" · "+c(v.data_venda):"")+(v.imei?" · IMEI "+c(v.imei):"")+"</div>"+fxVenda(v)+nfLinhaVenda(v)+"</div>"}
...
var fvLeadSel=null;function fvCliHit(a){return'<button type="button" class="fv-cli-hit" data-acao="fv-cli-pick" data-id="'+c(a.id)+'"><strong>'+c(a.nome||"(sem nome)")+'</strong><span>'+(a.whatsapp_digitos?c(f(a.whatsapp_digitos)):"sem telefone")+(a.produto?" · "+c(a.produto):"")+'</span></button>'}function buscaClienteVenda(){var cx=E("fvClienteResultados");if(!cx)return;var termo=E("fvClienteBusca")?E("fvClienteBusca").value:"";if(!String(termo).trim()){cx.innerHTML="";return}var ativos=(i||[]).filter(function(x){return!x.arquivado_em}),hits=g(ativos,termo).slice(0,6);cx.innerHTML=hits.length?hits.map(fvCliHit).join(""):'<div class="fv-cli-vazio">Nenhum cliente na base com esse termo.</div>'}function preencherClienteVenda(id){var L=(i||[]).filter(function(x){return String(x.id)===String(id)})[0];if(!L)return;fvLeadSel=L;if(E("fvNome"))E("fvNome").value=L.nome||"";if(E("fvWhats"))E("fvWhats").value=L.whatsapp_digitos?f(L.whatsapp_digitos):"";if(E("fvClienteResultados"))E("fvClienteResultados").innerHTML="";if(E("fvClienteBusca"))E("fvClienteBusca").value="";var st=E("fvLeadStatus");if(st)st.innerHTML='Vinculado a '+c(L.nome||L.lead_code||"lead")+(L.lead_code?' · '+c(L.lead_code):"")+' <button type="button" class="fv-cli-desfazer" data-acao="fv-cli-limpar">desfazer</button>'}function limparClienteVenda(){fvLeadSel=null;if(E("fvClienteBusca"))E("fvClienteBusca").value="";if(E("fvClienteResultados"))E("fvClienteResultados").innerHTML="";if(E("fvLeadStatus"))E("fvLeadStatus").innerHTML=""}function fvCliClick(ev){var el=ev.target&&ev.target.closest?ev.target.closest("[data-acao]"):null;if(!el)return;var o=el.getAttribute("data-acao");if("fv-cli-pick"===o)preencherClienteVenda(el.getAttribute("data-id"));else if("fv-cli-limpar"===o)limparClienteVenda()}async function salvarVenda(){
...
I(res.ok?"Venda "+d.venda_code+" registrada com NF":"Venda "+d.venda_code+" registrada, mas a NF não subiu: "+res.erro,!res.ok);
fa.value=""}
else I("Venda "+d.venda_code+" registrada");
fecharPainelVenda();n="vendas";k()}
