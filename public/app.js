...
function syncLinha(s){if(!s||null==s.ok)return'<div class="sync-lin">sync nunca rodou</div>';if(!1===s.ok)return'<div class="sync-lin falha">último sync falhou · '+c(s.msg||"")+"</div>";var v=s.horas>=24;return'<div class="sync-lin'+(v?" velho":"")+'">sincronizado há '+c(String(s.horas))+"h"+(v?" · atrasado":"")+"</div>"}async function qF(nome,args,btn,depois){var i=await O(nome,args,btn);i&&(!1!==i.ok?(I(i.msg||"Feito"),depois&&depois()):I(i.msg||"Operação recusada",!0))}function contItem(x){var sub=x.tipo_rotulo||x.semana?'<div class="cont-tipo">'+c(x.tipo_rotulo||"")+(x.semana?(x.tipo_rotulo?" · ":"")+c(x.semana):"")+"</div>":"";return'<div class="cont-lin"><div class="cont-tit">'+c(x.titulo||"")+sub+"</div>"+(x.status_rotulo?'<span class="chip">'+c(x.status_rotulo)+"</span>":"")+(x.url?'<a class="cont-link" target="_blank" rel="noopener" href="'+c(x.url)+'">Notion</a>':"")+"</div>"}function hojePlacar(d){var g=d.contagem||{},f=g.feitas||0,tt=g.total||0,pct=tt?Math.round(100*f/tt):0,ab=(d.lembretes||[]).filter(function(x){return!x.feito}).length,s=d.sync||{},sn,sp,sc="";null==s.ok?(sn="—",sp="sync nunca rodou"):!1===s.ok?(sn="—",sp="último sync falhou",sc=" falha"):(sn=s.horas+"h",sp="desde o último sync",s.horas>=24&&(sc=" velho"));return'<div class="pitboard">'+capCel("rotina",pct+"%",f+" de "+tt+" feitas","rotina")+capCel("conteúdo",(d.conteudo||[]).length,"peças hoje","conteudo")+capCel("lembretes",ab,"em aberto","lembretes")+'<div class="pb-celula"><div class="pb-rot">sync</div><div class="pb-num">'+c(sn)+'</div><div class="pb-pe'+sc+'">'+c(sp)+"</div></div></div>"}function hojeTarefas(d){var cats=d.categorias||[],corpo,ops="";cats.forEach(function(x){!1!==x.ativa&&(ops+='<option value="'+c(x.codigo)+'">'+c(x.rotulo)+"</option>")});corpo=cats.length?cats.map(function(ct){var ts=(ct.tarefas||[]).map(function(x){return'<div class="dia-lin"><button class="dia-tarefa" role="checkbox" aria-checked="'+(x.concluida?"true":"false")+'" data-acao="dia-marcar" data-id="'+c(x.id)+'"><span class="dia-check">'+svgCheck()+'</span><span class="dia-tit">'+c(x.titulo||"")+"</span></button><button class=\"dia-rm\" data-acao=\"dia-remover\" data-id=\""+c(x.id)+'">remover</button></div>'}).join("");return'<div class="dia-cat-rot" style="--tr:'+trilhoDe(ct.codigo)+'">'+iconeCat(ct.codigo)+c(ct.rotulo||ct.codigo)+"</div>"+(ts||'<div class="dia-vazio">Nada nesta categoria hoje.</div>')}).join(""):'<div class="dia-vazio">A rotina de hoje está vazia. O molde se digita na aba Rotina; o dia nasce dele de manhã, ou puxe agora.</div>';var add=ops?'<div class="dia-add"><input id="diaNovoTit" placeholder="Tarefa avulsa de hoje…" autocomplete="off"><select id="diaNovaCat" aria-label="Categoria">'+ops+'</select><button class="btn-acao" data-acao="dia-add">Adicionar</button></div>':"";var _g=d.contagem||{},_f=_g.feitas||0,_t=_g.total||0;return'<div class="dia-sec"><div class="dia-sec-cab"><div class="dia-sec-tit">Rotina do dia</div>'+'<span class="dia-sec-badge neutro">'+_f+' de '+_t+' feitas</span>'+'<button class="btn-nota" data-acao="dia-puxar">Puxar do molde</button></div>'+corpo+add+"</div>"}function hojeNota(d){return'<div class="dia-sec"><div class="dia-sec-tit">Nota do dia</div><div class="dia-nota"><textarea id="diaNota" placeholder="O que não pode se perder do dia…">'+c(d.nota||"")+'</textarea></div><div class="dia-nota-pe"><button class="btn-acao" data-acao="dia-nota-ok">Salvar nota</button></div></div>'}function nivelPonto(nv){return'<span class="fila-ponto n-'+c(nv||"quente")+'" aria-hidden="true"></span>'}var filaSug={};function filaEnviarHTML(a){if(!podeAbordar(a))return"";var h=filaSug[a.id];return h?'<a class="btn-wa fila-wa card-acao-principal" target="_blank" rel="noopener" href="'+h+'">Enviar</a>':""}function filaWaCard(a){var d=String(a.whatsapp_digitos||"").replace(/\D/g,"");if(!d)return'<div class="sem-tel card-wa-linha">Sem telefone na base</div>';if(!0!==a.consentimento)return'<div class="sem-tel card-wa-linha">Sem consentimento</div>';if(!podeAbordar(a))return'<div class="sem-tel card-wa-linha">Contato pausado pela regua</div>';var h=(filaSug||{})[a.id]||"https://wa.me/"+d;return'<a class="btn-wa card-wa-linha card-acao-principal" target="_blank" rel="noopener" data-wa-lead="'+c(a.id)+'" href="'+c(h)+'"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 20l1.3-4A8 8 0 1 1 8 18.7L4 20z" stroke-linecap="round" stroke-linejoin="round"></path></svg>Chamar no WhatsApp</a>'}function filaWaAtualizar(){[].forEach.call(document.querySelectorAll("[data-wa-lead]"),function(el){var h=(filaSug||{})[el.getAttribute("data-wa-lead")];if(h)el.setAttribute("href",h)})}async function prefetchFilaSug(lim){if(!filaSug)filaSug={};var ativos=(i||[]).filter(function(x){return!x.arquivado_em}),top=vOperacional(ativos,l()).slice(0,lim||5);await Promise.all(top.map(async function(a){if(!podeAbordar(a))return;if(filaSug[a.id])return;var dig=String(a.whatsapp_digitos||"").replace(/\D/g,"");if(!dig)return;try{var res=await t.rpc("sugerir_mensagem",{p_lead_id:a.id});var dd=res&&res.data;if(!dd||!1===dd.ok)return;var ops=dd.opcoes||[];if(!ops.length||!ops[0]||!ops[0].texto)return;filaSug[a.id]="https://wa.me/"+dig+"?text="+encodeURIComponent(ops[0].texto)}catch(e){}}))}function filaResultadoHTML(a,hj){if(!a||!a.id||!podeAbordar(a))return"";var g=c(a.id);return'<div class="fila-fluxo"><span class="fila-registro"><button class="btn-acao toque fila-toque" data-acao="toque" data-id="'+g+'">Toque enviado</button><button class="btn-acao fila-reg-btn" data-acao="leque" data-id="'+g+'" aria-expanded="false">Registrar resultado</button></span></div><div class="scripts" data-scripts></div><div class="desfechos"><div class="fila-desfecho-tit">O que aconteceu?</div><button class="btn-desf respondeu" data-acao="respondeu" data-id="'+g+'">Respondeu</button><button class="btn-desf" data-acao="conversando" data-id="'+g+'">Conversando</button><button class="btn-desf" data-acao="retomar" data-id="'+g+'">Retomar</button><button class="btn-desf ok" data-acao="fechou" data-id="'+g+'">Fechou</button><button class="btn-desf frio" data-acao="sem-interesse" data-id="'+g+'">Sem interesse</button></div><div class="retomar"><input type="date" value="'+c(C(hj,1))+'" aria-label="Retomar em"><button class="btn-desf" data-acao="retomar-ok" data-id="'+g+'">Confirmar</button></div>'}function hojeFilaLin(a){var nv=a.nivel||"quente",hj=l(),atr=p(a.proximo_contato,hj),prod=c(a.produto||"")+(a.condicao?" · "+c(s("condicao",a.condicao)):"sem produto");return'<div class="fila-lin" data-lead="'+c(a.lead_code||"")+'"><div class="fila-lin-topo"><button class="btn-acao sugerir fila-sug fila-msg" data-acao="hoje-sugerir" data-id="'+c(a.id)+'">Enviar mensagem</button><div class="fila-ident">'+nivelPonto(nv)+'<span class="fila-nome">'+c(a.nome||"")+'</span><span class="fila-nivel n-'+c(nv)+'">'+c(s("nivel",nv))+'</span></div>'+fxVerChip(a,atr,!0)+'</div><div class="fila-contexto"><span>'+prod+'</span><span>'+(a.perfil?c(s("perfil",a.perfil)):"sem perfil")+"</span></div>"+fxOperacao(a,hj,atr)+fxMotivo(a,!0)+filaResultadoHTML(a,hj)+'</div>'}function hojeFila(d){var ativos=(i||[]).filter(function(x){return!x.arquivado_em}),fila=vOperacional(ativos,l()),recorte=filaRecorte(fila,l());if(!fila.length)return'<div class="dia-sec"><div class="dia-sec-cab fila-hoje-cab"><div class="dia-sec-tit">Fila de hoje</div></div>'+recorte+'<div class="dia-vazio">Fila zerada hoje. Nada vencendo.</div></div>';var top=fila.slice(0,5).map(hojeFilaLin).join("");return'<div class="dia-sec"><div class="dia-sec-cab fila-hoje-cab"><div class="dia-sec-tit">Fila de hoje</div><button class="fila-vertodos" data-acao="hoje-verfila" aria-label="Ver todos os '+fila.length+' itens da fila">ver todos</button></div>'+recorte+top+"</div>"}function hojeLembretes(d){var ls=(d.lembretes||[]).map(function(x){var _lc="dia-lin"+(x.vencido?" lemb-vencido":x.agendado?" lemb-agendado":""),_lt=x.vencido?'<span class="lemb-tag venc">'+(p(x.data,l())<=1?"venceu ontem":"atrasado "+p(x.data,l())+" dias")+"</span>":x.agendado?'<span class="lemb-tag agnd">agendado pra hoje</span>':"";return'<div class="'+_lc+'"><button class="dia-tarefa" role="checkbox" aria-checked="'+(x.feito?"true":"false")+'" data-acao="lemb-marcar" data-id="'+c(x.id)+'"><span class="dia-check">'+svgCheck()+'</span><span class="dia-tit">'+c(x.texto||"")+"</span>"+_lt+"</button><button class=\"dia-rm\" data-acao=\"lemb-remover\" data-id=\""+c(x.id)+'">remover</button></div>'}).join("");return'<div class="dia-sec"><div class="dia-sec-tit">Lembretes</div>'+(ls||'<div class="dia-vazio">Nenhum lembrete para hoje.</div>')+'<div class="dia-add"><input id="lembNovo" placeholder="Novo lembrete…" autocomplete="off"><input type="date" id="lembData" value="'+c(l())+'" aria-label="Dia do lembrete"><button class="btn-acao" data-acao="lemb-add">Adicionar</button></div><div class="lemb-quando"><button class="lemb-q" data-acao="lemb-hoje">Hoje</button><button class="lemb-q" data-acao="lemb-amanha">Amanha</button></div></div>'}// Card RETANGULAR do dia. A aba Hoje e onde o dia se confere e se fecha, entao a
...
function hojeConteudo(d){var l=d.conteudo||[],itens=l.map(hojeContCard).join("");return'<div class="dia-sec"><div class="dia-sec-cab"><div class="dia-sec-tit">Conteúdo de hoje</div>'+(l.length?hojeContPlacar(l):"")+syncLinha(d.sync)+"</div>"+(itens?'<div class="dia-cont-lista">'+itens+"</div>":'<div class="dia-vazio">Nada no calendário para hoje.</div>')+"</div>"}
// ---- aba Hoje: as tres leituras do topo (referencia de 19/08/2026) ----------
// Tudo aqui e DERIVADO na leitura, do dado que a aba ja tem em memoria: nivel,
// origem e data de entrada saem de `i` (leads), dinheiro sai de vendasData.
// Nenhum numero novo e gravado, e nenhuma RPC nova entra.
function hjDiaCurto(d){
if(!d)return"";var p=String(d).split("-");return p.length<3?String(d):p[2]+"/"+p[1]}
var HJ_ICO={
tend:'<path d="M3 17l5-5 4 3 6-7"/><path d="M21 8h-4V4"/>',
origem:'<circle cx="12" cy="12" r="9"/><path d="M12 3v9l7.8 4.5"/>',
temp:'<path d="M10 13V5a2 2 0 1 1 4 0v8"/><circle cx="12" cy="16.5" r="3.5"/>'};
function hjCard(tit,jan,ico,corpo,cls){
return'<section class="hj-card'+(cls?" "+cls:"")+'"><div class="hj-card-cab">'+
'<span class="hj-card-ico" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">'+(HJ_ICO[ico]||"")+'</svg></span>'+
'<h2 class="hj-card-tit">'+c(tit)+'</h2><span class="hj-card-jan">'+c(jan)+"</span></div>"+corpo+"</section>"}
function hjLeadsVivos(){
var out=[],k;
for(k=0;k<(i||[]).length;k++)if(!i[k].arquivado_em)out.push(i[k]);
return out}
// Lead novo por dia nos ultimos 7. O mock desenha barra de lead + linha de
// conversao; a conversao diaria com este volume seria uma linha de zeros, entao
// entra so a barra, que e o que o dado sustenta.
function hjVendasPorDia(){
var mapa={},k,v,d;
for(k=0;k<(vendasData||[]).length;k++){
v=vendasData[k];
if(v.arquivado_em||!vgConta(v))continue;
d=vgDataDe(v);
if(d)mapa[d]=(mapa[d]||0)+1}
return mapa}
// As DUAS series da referencia: barra = lead novo, linha = venda fechada.
// A referencia chama a segunda de "conversao" em percentual; conversao por DIA
// com este volume seria uma linha de zeros com nome de taxa, entao a serie diz o
// que ela e (venda fechada no dia) e a legenda nomeia as duas.
// Escala UNICA para as duas: escala propria por serie faz 1 venda parecer do
// tamanho de 5 leads, que e como grafico de duas series mente.
function hojeTendencia(){
var ds=[],k,cl={},cv=hjVendasPorDia(),mx=0,tot=0,totV=0,ls=hjLeadsVivos(),dd;
for(k=6;k>=0;k--)ds.push(C(l(),-k));
for(k=0;k<ls.length;k++){dd=u(ls[k].criado_em);if(dd)cl[dd]=(cl[dd]||0)+1}
for(k=0;k<ds.length;k++){
tot+=cl[ds[k]]||0;totV+=cv[ds[k]]||0;
if((cl[ds[k]]||0)>mx)mx=cl[ds[k]];
if((cv[ds[k]]||0)>mx)mx=cv[ds[k]]}
// Tres grades de SETE alinhadas: numero em cima, desenho no meio, dia embaixo.
// O numero fica em HTML e nao dentro do SVG porque texto dentro de viewBox
// escala junto com a largura do card, e ai o mesmo grafico teria fonte
// diferente em cada largura de tela.
var A=170,base=A-10,teto=6,esc=mx>0?(base-teto)/mx:0;
var barras="",pts=[],pontos="",nums="",dias="";
for(k=0;k<7;k++){
var nL=cl[ds[k]]||0,nV=cv[ds[k]]||0,eHoje=6===k;
var hL=Math.round(nL*esc),cx=k*100+50;
barras+='<rect class="hj-graf-barra'+(eHoje?" hoje":"")+'" x="'+(k*100+14)+'" y="'+(nL?base-hL:base-3)+'" width="72" height="'+(nL?hL:3)+'" rx="4"/>';
pts.push(cx+","+(base-Math.round(nV*esc)));
if(nV)pontos+='<circle class="hj-graf-ponto" cx="'+cx+'" cy="'+(base-Math.round(nV*esc))+'" r="7"/>';
nums+='<span'+(nL?"":' class="zero"')+">"+nL+"</span>";
var dj=new Date(ds[k]+"T12:00:00").getDay();
dias+="<span"+(eHoje?' class="hoje"':"")+">"+c(eHoje?"hoje":DIAS_ISO[0===dj?7:dj])+"</span>"}
var svg='<svg class="hj-graf-svg" viewBox="0 0 700 '+A+'" preserveAspectRatio="xMidYMid meet" aria-hidden="true">'+
barras+'<polyline class="hj-graf-linha" points="'+pts.join(" ")+'" vector-effect="non-scaling-stroke"/>'+pontos+"</svg>";
return hjCard("Leads novos",hjDiaCurto(ds[0])+" a "+hjDiaCurto(ds[6]),"tend",
'<div class="hj-graf-leg"><span class="hj-leg"><i class="hj-leg-barra"></i>Leads</span>'+
'<span class="hj-leg"><i class="hj-leg-linha"></i>Vendas</span></div>'+
'<div class="hj-graf"><div class="hj-graf-nums">'+nums+"</div>"+svg+
'<div class="hj-graf-dias">'+dias+"</div></div>"+
'<p class="hj-card-pe">'+tot+" lead"+(1===tot?"":"s")+" \u00b7 "+totV+" venda"+(1===totV?"":"s")+" em 7 dias</p>","hj-2")}
// De onde vem, na BASE VIVA (todo lead nao arquivado). E outra janela que a do
// card do Dashboard, que conta so quem entrou no periodo: por isso o cabecalho
// diz "base viva" em vez de deixar o leitor supor.
function hojeOrigemHj(){
var ls=hjLeadsVivos(),mapa={},k,cod,arr=[],tot=ls.length;
for(k=0;k<ls.length;k++){
cod=String(ls[k].origem||"").trim()||"sem_origem";
mapa[cod]=(mapa[cod]||0)+1}
for(cod in mapa)if(Object.prototype.hasOwnProperty.call(mapa,cod))arr.push({cod:cod,n:mapa[cod]});
arr.sort(function(a,b){return b.n-a.n});
var corpo=arr.length?arr.map(function(x){
var pct=tot>0?Math.round(100*x.n/tot):0;
return'<div class="org-lin org-lin-min"><div class="org-lin-topo"><span class="org-rot">'+
c("sem_origem"===x.cod?"Sem origem":s("origem",x.cod))+'</span><span class="org-pct">'+pct+'%</span></div>'+
'<div class="org-barra"><i style="width:'+(x.n>0&&pct<2?2:pct)+'%"></i></div></div>'}).join(""):
'<div class="hj-vazio">Nenhum lead na base.</div>';
return hjCard("De onde vem","base viva \u00b7 "+tot+" lead"+(1===tot?"":"s"),"origem",corpo,"hj-2")}
// Temperatura: o nivel e DERIVADO na leitura (invariante 4), entao aqui ele so
// e contado, nunca recalculado nem guardado.
var HJ_NIVEIS=["quente","morno","frio","sem_contato"];
function hojeTemperatura(){
var ls=hjLeadsVivos(),ct={},k,nv,tot=0,celulas="",faixa="";
for(k=0;k<ls.length;k++){nv=String(ls[k].nivel||"");if(!nv)continue;ct[nv]=(ct[nv]||0)+1;tot++}
if(!tot)return hjCard("Temperatura","base viva","temp",'<div class="hj-vazio">Nenhum lead na base.</div>',"hj-2");
for(k=0;k<HJ_NIVEIS.length;k++){
nv=HJ_NIVEIS[k];
if(!ct[nv])continue;
var pct=Math.round(100*ct[nv]/tot);
celulas+='<div class="hj-tp-cel n-'+nv+'"><div class="hj-tp-num">'+pct+'%</div>'+
'<div class="hj-tp-rot">'+c(s("nivel",nv))+'</div>'+
'<div class="hj-tp-abs">'+ct[nv]+" lead"+(1===ct[nv]?"":"s")+"</div></div>";
faixa+='<i class="n-'+nv+'" style="flex-grow:'+ct[nv]+'" title="'+c(s("nivel",nv))+": "+ct[nv]+'"></i>'}
return hjCard("Temperatura","base viva \u00b7 "+tot+" lead"+(1===tot?"":"s"),"temp",
'<div class="hj-tp">'+celulas+'</div><div class="hj-tp-faixa">'+faixa+"</div>","hj-2")}
// Dinheiro do MES corrente contra o mes anterior fechado. Reusa vgAgregar e
// dvKpi do Dashboard de proposito: dois calculos do mesmo numero em duas abas e
// como nasce divergencia que ninguem consegue auditar depois.
function hjPct(atual,ant){
var d=100*(atual-ant)/Math.abs(ant);
return(d>=0?"+":"\u2212")+vgPct1(Math.abs(d),100)}
// Card de dinheiro no formato da referencia: a VARIACAO e o numero grande, e o
// valor absoluto vira rodape. Sem mes anterior nao existe variacao, e o card
// mostra o R$ em vez de inventar um zero: comparar contra base inexistente e
// mentira, e a referencia nao previu este estado porque os numeros dela eram
// fixos.
function hjKpiPct(cod,rot,ico,tom,atual,ant,temAnt,ymAnt,pe){
var num,cls="";
if(temAnt){num=hjPct(atual,ant);cls=atual>=ant?" v-bom":" v-ruim"}
else num=brlV(atual);
return'<article class="kp hj-kpi" data-kpi="'+c(cod)+'"><div class="kp-topo">'+
'<p class="kp-rot">'+c(rot)+"</p>"+dvIcone(ico,tom)+"</div>"+
'<p class="kp-num hj-kpi-num'+cls+'">'+c(num)+"</p>"+
'<p class="kp-pe">'+(temAnt?'<span class="hj-kpi-base">'+brlV(atual)+'</span><span class="kp-vs">vs '+c(ymAnt)+"</span>":'<span class="kp-var kp-sem">'+pe+"</span>")+"</p></article>"}
function hojeDinheiro(){
var linhas=vendasData||[],ym=l().slice(0,7),ymA=vgMesMais(ym,-1);
var a=vgAgregar(linhas,{ini:ym+"-01",fim:l()});
var b=vgAgregar(linhas,{ini:ymA+"-01",fim:dvFimMes(ymA)});
var temAnt=b.n>0&&b.fat>0,rotA=vgMesRot(ymA);
return hjKpiPct("fat","Faturamento do m\u00eas","dinheiro","azul",a.fat,b.fat,temAnt,rotA,
  a.n+(1===a.n?" venda":" vendas")+" no m\u00eas")+
hjKpiPct("luc","Lucro do m\u00eas","lucro","verde",a.luc,b.luc,temAnt&&0!==b.luc,rotA,
  a.n?brlV(a.luc)+" no m\u00eas":"nada fechado")}
// Duas linhas de QUATRO unidades, como no Dashboard: a peca larga (grafico,
// que precisa de eixo de 7 dias) pede 2, e a lista curta pede 2 tambem porque
// abaixo disso o rotulo de origem trunca. Nao sobra unidade em nenhuma linha.
function hojeLeitura(dinheiro){
return'<div class="hj-l">'+dinheiro+hojeTemperatura()+"</div>"+
'<div class="hj-l">'+hojeTendencia()+hojeOrigemHj()+"</div>"}

// ---- PENDENCIAS: o que esta aberto e atrasado, de todos os trilhos ---------
// Cada linha e contagem DERIVADA na leitura, nunca guardada. O botao nao
// resolve a pendencia: leva para a aba onde ela se resolve, porque resolver
// aqui exigiria duplicar em miniatura a acao de cinco telas diferentes.
function hjPendLin(cod,rot,n,aba,urgente){
return'<div class="pend-lin"><span class="pend-num'+(urgente?" urg":"")+'">'+n+"</span>"+
'<span class="pend-rot">'+c(rot)+"</span>"+
(aba?'<button class="pend-ir" data-acao="pend-ir" data-aba="'+c(aba)+'">abrir</button>':"")+
"</div>"}
function hjEspera(h){var n=Number(h)||0;return n<48?Math.round(n)+"h":Math.round(n/24)+"d"}
function hojePendencias(d){
var hj=l(),ativos=hjLeadsVivos(),linhas="",k,n;
var fila=v(ativos,hj),atrasados=0;
for(k=0;k<fila.length;k++)if(p(fila[k].proximo_contato,hj)>0)atrasados++;
if(atrasados)linhas+=hjPendLin("fila","lead"+(1===atrasados?"":"s")+" com contato atrasado",atrasados,"abaFila",!0);
var esp=0,pior=0,he;
...
if(esp)linhas+=hjPendLin("espera","sem 1\u00ba toque \u00b7 o mais antigo h\u00e1 "+hjEspera(pior),esp,"abaFila",pior>=24);
var pos=pitscareGrupos(ativos,hj).venc.length;
if(pos)linhas+=hjPendLin("pos","cliente"+(1===pos?"":"s")+" com p\u00f3s-venda vencido",pos,"abaPitscare",!0);
var vs=vendasData||[],ent=0,semnf=0;
for(k=0;k<vs.length;k++){
if(vs[k].arquivado_em||"concluida"!==vs[k].status)continue;
if("entregue"!==String(vs[k].etapa||""))ent++;
if(!String(vs[k].nf_numero||"").trim()&&!String(vs[k].nf_url||"").trim())semnf++}
if(ent)linhas+=hjPendLin("entrega","entrega"+(1===ent?"":"s")+" sem baixa",ent,"abaVendas",!0);
if(semnf)linhas+=hjPendLin("nf","venda"+(1===semnf?"":"s")+" sem nota fiscal",semnf,"abaNfs");
var cont=(d.conteudo||[]),falta=0;
for(k=0;k<cont.length;k++)if("publicado"!==cont[k].status_codigo&&"descartado"!==cont[k].status_codigo)falta++;
if(falta)linhas+=hjPendLin("cont","pe\u00e7a"+(1===falta?"":"s")+" de hoje sem publicar",falta,"abaConteudo");
var lemb=(d.lembretes||[]),venc=0;
for(k=0;k<lemb.length;k++)if(lemb[k].vencido)venc++;
if(venc)linhas+=hjPendLin("lemb","lembrete"+(1===venc?"":"s")+" vencido",venc,"",!0);
var g=d.contagem||{};
n=(g.total||0)-(g.feitas||0);
if(n)linhas+=hjPendLin("rotina","tarefa"+(1===n?"":"s")+" da rotina em aberto",n,"");
return'<div class="dia-sec pend-sec"><div class="dia-sec-cab"><div class="dia-sec-tit">Pend\u00eancias</div></div>'+
(linhas?'<div class="pend-grade">'+linhas+"</div>":
'<div class="dia-vazio">Nada atrasado em nenhum trilho. O dia est\u00e1 limpo.</div>')+"</div>"}
async function renderHoje(sil){var e=E("lista");if(!sil)e.innerHTML='<div class="estado carregando">Lendo o dia…</div>';var r=await t.rpc("painel_do_dia",{});if(r.error)return void(e.innerHTML='<div class="estado erro">Falha ao ler o dia: '+c(r.error.message)+". Toque em Atualizar para tentar de novo.</div>");var d=r.data;if(!d||!1===d.ok)return void(e.innerHTML='<div class="estado erro">'+c(d&&d.msg||"Falha ao ler o dia.")+"</div>");await prefetchFilaSug();var rv=await carregarVendas();if(rv&&!1===rv.sessao)return;var dinheiro=rv&&rv.ok?hojeDinheiro():estadoErro("as vendas",rv?rv.erro:"");e.innerHTML=hojeLeitura(dinheiro)+hojePendencias(d)+hojeFila(d)+'<div class="hj-grid"><div class="hj-col">'+hojeTarefas(d)+'</div><div class="hj-col">'+hojeConteudo(d)+hojeLembretes(d)+hojeNota(d)+'</div></div>'+'<div class="hj-rodape">'+reguaLinha(d.regua)+'</div>'}
var CONT_COLUNAS=[
{cod:"a_produzir",rot:"A produzir"},
{cod:"em_producao",rot:"Em produção"},
{cod:"pronto",rot:"Pronto"},
{cod:"publicado",rot:"Publicado"}];
// ---- Atrasados: coluna DERIVADA, nunca status (01/09/2026) ---------------
// Pedido do dono: "uma aba de conteudos atrasados no kanban, que passe
// automaticamente os conteudos ao atrasar a data de publicado e nao marcado".
// Passa mesmo, e sozinha: nada empurra o card para ca. Ele cai aqui porque
// nivelPeca() devolveu "vencido" (data < hoje no fuso do Brasil, invariante 10,
// e status diferente de publicado e de descartado).
//
// Por que NAO entra em CONT_COLUNAS: o Notion e fonte unica e la nao existe
// status "Atrasado". Se entrasse, o menu Mover ofereceria "Atrasados" e o
// PATCH mandaria um valor que a base do Notion recusa. E pelo invariante 4
// atraso e LEITURA, nunca coluna armazenada: aparece sozinho e some sozinho
// quando o dono publica, descarta ou remarca a Data no Notion.
//
// A conta que isto cobra da tela: enquanto o card mora aqui, a POSICAO dele
// deixa de dizer a etapa, que era o unico lugar onde o kanban guardava essa
// informacao. Por isso o cartao atrasado carrega o rotulo da etapa real
...
// payload antigo que nao trazia o campo: rotulo e display e editavel, entao
// quem manda e o codigo (invariante 12).
function contRotEtapa(x){
var r=String(x.status_rotulo||"").trim(),i;
if(r)return r;
for(i=0;i<CONT_COLUNAS.length;i++)if(CONT_COLUNAS[i].cod===x.status_codigo)return CONT_COLUNAS[i].rot;
return"sem etapa"}
...
// Na coluna Atrasados a posicao ja nao diz a etapa: o cartao diz.
var etp="vencido"===nv?'<div class="cont-etapa">'+c(contRotEtapa(x))+"</div>":"";
var mvAtt=mover?' draggable="true" data-id="'+c(x.id)+'" data-col="'+c(x.status_codigo)+'"':"";var mvCtl=mover?contMoverCtl(x):"";return'<div class="cont-card nivel-'+nv+' tipo-'+c(x.tipo_codigo||"outro")+'"'+mvAtt+' style="--tp:'+tipoDe(x.tipo_codigo)+'"><div class="cont-data-chip">'+c(fmtDia(x.data))+("vencido"===nv?'<span class="cont-venc">vencida</span>':"")+'</div><div class="cont-tit">'+c(x.titulo||"")+"</div>"+sub+etp+(x.url?'<a class="cont-link" target="_blank" rel="noopener" href="'+c(x.url)+'">Notion</a>':"")+contMetrica(x)+mvCtl+"</div>"}
// O selo "N vencidas" no cabecalho saiu em 01/09/2026 junto com a coluna
// Atrasados: ele existia para denunciar atraso ESCONDIDO dentro de uma coluna
// de etapa, e atraso agora tem coluna propria com contador proprio. Manter os
// dois seria contar a mesma divida duas vezes, em dois lugares livres para
// divergir. O marcador da coluna fica no data-col, nao numa classe nova:
...
var kan='<div class="cont-kanban">'+[CONT_ATRASADO].concat(CONT_COLUNAS).map(function(col){return contColuna(col,itens)}).join("")+"</div>";
...
var METRICA_DIAS=90;
function svgOrigem(){
return'<svg class="org-ico" viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="9"/><path d="M12 3v9l7.8 4.5"/></svg>'}
function metLinOrigem(x,tot){
var pct=tot>0?Math.round(100*x.leads/tot):0;
return'<div class="org-lin"><div class="org-lin-topo"><span class="org-rot">'+c(x.rotulo)+'</span><span class="org-pct">'+pct+'%</span></div>'+
'<div class="org-barra"><i style="width:'+(x.leads>0&&pct<2?2:pct)+'%"></i></div>'+
'<div class="org-pe"><span>'+fmtNum(x.leads)+" lead"+(1===x.leads?"":"s")+'</span><span>'+fmtNum(x.clientes)+" cliente"+(1===x.clientes?"":"s")+'</span><span>'+x.taxa+"% viraram</span>"+
'<span class="org-val">'+brlV(x.valor_venda)+"</span>"+
(Number(x.valor_historico)>0?'<span class="org-hist">histórico '+brlV(x.valor_historico)+"</span>":"")+"</div></div>"}
function metOrigem(d){
var it=((d.origem||{}).itens||[]).slice(),tt=(d.origem||{}).total||{},tot=0,i;
for(i=0;i<it.length;i++)tot+=it[i].leads;
it.sort(function(a,b){return b.leads-a.leads||Number(b.valor_venda||0)-Number(a.valor_venda||0)});
var corpo=it.length?it.map(function(x){return metLinOrigem(x,tot)}).join(""):
'<div class="estado"><strong>Nenhum lead nesta janela.</strong>Amplie o período acima.</div>';
return'<section class="met-sec"><div class="org-card"><div class="org-cab">'+svgOrigem()+
'<h2 class="org-tit">De onde veio</h2><span class="org-janela">'+c(hjDiaCurto(d.ini))+" a "+c(hjDiaCurto(d.fim))+"</span></div>"+
'<div class="org-tot">'+fmtNum(tt.leads||0)+" leads · "+fmtNum(tt.clientes||0)+" viraram cliente</div>"+
'<p class="met-nota">O <strong>%</strong> é a fatia dos leads da janela. Conta o lead cujo <strong>primeiro contato</strong> caiu nela, e o R$ que ele gerou mesmo que a venda tenha saído depois.</p>'+
corpo+
'<p class="met-rodape">R$ vendido soma a tabela <code>venda</code>, linha a linha. O “histórico” é o agregado herdado do CRM antigo, sem lastro por venda: os dois não se somam.</p></div></section>'}
...
// Spec: docs/superpowers/specs/2026-08-25-financeiro-design.md
// ==========================================================================
// A dor que esta aba existe para responder foi dita pelo dono em cinco
// palavras: "nao sei onde o dinheiro vai". Por isso a Visao abre em SECAO DE
// GASTO, e nao em lista de lancamento: lista responde "o que aconteceu", e a
// pergunta dele e "para onde foi". A lista existe, mas na sub-view Movimentos,
// que e onde se TRABALHA o dado, nao onde se le o resultado.
//
// INVARIANTE 18: movimento sem `dominio` classificado nao entra em total
// nenhum. A faixa do topo e a metade visivel desse invariante. Sem ela o
// placar pareceria completo estando incompleto, que e a mesma mentira por
// omissao que a aba Conteudo consertou declarando a janela.
//
// Nada de categoria, conta ou grupo chumbado aqui: tudo vem de fin_config()
// (invariantes 11 e 12). Categoria nova entra no banco e aparece na tela sem
// deploy nenhum.
//
...
// Vida dividem o mesmo matiz, e quem os separa e o ICONE (megafone x xicara),
// exatamente como o sistema Trilho x Sinal resolve as colisoes de luminancia de
// 1.14 a 1.44 entre trilho e cor semantica. Trilho sem icone e regressao.
// O par foi escolhido por serem os dois icones mais distantes entre si e por
// viverem em lados opostos do dominio (empresa x pessoal).
//
// "Sem categoria" e a excecao e NAO e trilho: e ESTADO (trabalho pendente), e
// por isso usa --morno, o mesmo token que `nf-falta` ja usa para cadastro
// incompleto. Nunca --erro: gasto sem categoria e trabalho que falta, nao falha
// de sistema.
//
// A chave e o `grupo` que a RPC devolve, o unico identificador que existe para
// grupo. Grupo desconhecido NAO vira buraco: cai no anel do trilhoDe (hash
// deterministico do codigo) com icone generico.
var FIN_GRUPO={
"Mercadoria":["--tr-loja-estoque",'<path d="M4.5 8h15l-1.1 11.4a1.8 1.8 0 0 1-1.8 1.6H7.4a1.8 1.8 0 0 1-1.8-1.6L4.5 8z" stroke-linejoin="round"/><path d="M8.8 8a3.2 3.2 0 0 1 6.4 0" stroke-linecap="round"/>'],
"Operação":["--tr-fila-follow-up",'<circle cx="12" cy="12" r="3.2"/><path d="M12 3.2v2.4M12 18.4v2.4M4.4 7.6l2.1 1.2M17.5 15.2l2.1 1.2M4.4 16.4l2.1-1.2M17.5 8.8l2.1-1.2" stroke-linecap="round"/>'],
"Taxas":["--tr-fechamento",'<path d="M6.5 17.5l11-11" stroke-linecap="round"/><circle cx="8" cy="8" r="2.2"/><circle cx="16" cy="16" r="2.2"/>'],
"Marketing":["--tr-conteudo",'<path d="M4 10v4h3l7 4V6l-7 4H4z" stroke-linejoin="round"/><path d="M17.5 9a4 4 0 0 1 0 6" stroke-linecap="round"/>'],
"Outros":["--tr-analise",'<rect x="3.5" y="5" width="17" height="14" rx="2"/><circle cx="8.5" cy="12" r="1.1"/><circle cx="12" cy="12" r="1.1"/><circle cx="15.5" cy="12" r="1.1"/>'],
"Receita":["--tr-pos-venda",'<path d="M12 3.5v10M8 10l4 4 4-4" stroke-linecap="round" stroke-linejoin="round"/><path d="M4 16.5V19a1.6 1.6 0 0 0 1.6 1.6h12.8A1.6 1.6 0 0 0 20 19v-2.5" stroke-linecap="round"/>'],
"Casa":["--tr-captacao",'<path d="M3.8 11.2L12 4.6l8.2 6.6" stroke-linecap="round" stroke-linejoin="round"/><path d="M6 10.4V20h12v-9.6" stroke-linejoin="round"/><path d="M10 20v-4.6h4V20" stroke-linejoin="round"/>'],
"Vida":["--tr-conteudo",'<path d="M5 7.5h11v5.2a5.5 5.5 0 0 1-11 0V7.5z" stroke-linejoin="round"/><path d="M16 9h1.9a2.2 2.2 0 0 1 0 4.4H16" stroke-linecap="round"/><path d="M4 20.2h13" stroke-linecap="round"/>'],
"Neutro":["--tr-analise",'<path d="M4 9.2h13l-3.2-3.2M20 14.8H7l3.2 3.2" stroke-linecap="round" stroke-linejoin="round"/>'],
"Sem categoria":["--morno",'<circle cx="12" cy="12" r="8.4"/><path d="M9.7 9.6a2.4 2.4 0 1 1 3.2 2.3c-.7.3-.9.8-.9 1.5v.3" stroke-linecap="round"/><path d="M12 16.9v.2" stroke-linecap="round"/>']};
function finTrilho(gr){
var x=FIN_GRUPO[String(gr||"")];
return x?"var("+x[0]+")":trilhoDe(String(gr||""))}
function finIconeGrupo(gr){
var x=FIN_GRUPO[String(gr||"")];
return'<svg class="tr-ico" viewBox="0 0 24 24" aria-hidden="true">'+(x?x[1]:'<circle cx="12" cy="12" r="7"/>')+"</svg>"}
function finMes(){return FIN_MES||l().slice(0,7)}
// O fim para em HOJE no mes corrente, nunca no ultimo dia do mes: comparar 25
// dias contra 31 do mes anterior e a maneira mais barata de a tela mentir.
function finJanela(){
var ym=finMes(),ini=ym+"-01",fim=dvFimMes(ym),hj=l();
if(ini<=hj&&fim>hj)fim=hj;
return{ym:ym,ini:ini,fim:fim}}
function finN1(x){return Number(x||0).toFixed(1).replace(".",",")}
// delta_pct null quer dizer "nao havia base no periodo anterior". Desenhar 0%
// ali seria inventar uma comparacao que nunca existiu, entao a tela diz `novo`.
// O sinal e NEUTRO de proposito: gasto que sobe nao e necessariamente ruim
// (comprar estoque sobe Mercadoria), e pintar de vermelho seria a tela julgando
// o que ela nao tem como saber.
function finDelta(x){
if(null==x)return'<span class="fin-delta nova">novo</span>';
var v=Number(x);
return'<span class="fin-delta">'+(v>=0?"+":"−")+finN1(Math.abs(v))+"% vs anterior</span>"}
// Barra de MAGNITUDE, um tom so, nunca colorida por categoria: a altura ja
// carrega a mensagem, e barra colorida por categoria ja foi reprovada duas
// vezes neste projeto. --dim cheio mede 5.43 contra --surface, alvo 3.
// Fatia visivel minima de 2%: 0,4% que desenha nada le como "nao existe".
function finBarra(pct){
var v=Number(pct)||0;
if(v>0&&v<2)v=2;
...
'<span class="fin-sec-rot">'+c(sc.grupo||"")+"</span>"+
...
var doms=FIN_DOMS.map(function(x){
return'<button class="fin-chip" data-acao="fin-dom" data-dom="'+x[0]+'" aria-pressed="'+(FIN_DOM===x[0]?"true":"false")+'">'+c(x[1])+"</button>"}).join("");
return'<div class="fin-topo">'+
'<div class="fin-sub" role="group" aria-label="Seção do financeiro">'+subs+"</div>"+
'<div class="fin-dom" role="group" aria-label="Domínio">'+doms+"</div>"+
'<div class="fin-mes">'+
'<button class="fin-mes-b" data-acao="fin-mes" data-delta="-1" aria-label="Mês anterior">‹</button>'+
...
// vazia.
function finVazio(){
return'<div class="estado"><strong>Nenhum lançamento nesta janela.</strong>'+
...
// aba, e painel global para isso engorda o HTML de todo mundo.
// O sinal e SELETOR, nao um menos digitado: valor com sinal e o dado que manda
// (saida e negativo), e depender do dono lembrar do tracinho e convidar erro
// exatamente no campo que decide se o mes fechou no azul.
function finFormLanc(){
if(!FIN_LANC)return'<button class="btn-cad" data-acao="fin-lanc-abrir">+ lançamento manual</button>';
var ops=((FIN_CFG&&FIN_CFG.contas)||[]).map(function(x){
return'<option value="'+c(x.id)+'">'+c(x.rotulo)+"</option>"}).join("");
return'<div class="fin-form"><p class="cad-grupo">Lançamento manual</p>'+
'<p class="cad-nota">Dinheiro vivo não aparece no OFX. O que entra aqui vale igual ao que veio do extrato.</p>'+
'<div class="campo"><label for="finLancDesc">Descrição *</label><input id="finLancDesc" type="text" autocomplete="off" placeholder="CAFÉ DINHEIRO"></div>'+
'<div class="fin-form-lin">'+
'<div class="campo"><label for="finLancSinal">Tipo *</label><select id="finLancSinal"><option value="saida">Saída</option><option value="entrada">Entrada</option></select></div>'+
'<div class="campo"><label for="finLancValor">Valor (R$) *</label><input id="finLancValor" type="number" inputmode="decimal" step="0.01" min="0" autocomplete="off"></div>'+
'<div class="campo"><label for="finLancData">Data</label><input id="finLancData" type="date" value="'+c(l())+'"></div></div>'+
'<div class="campo"><label for="finLancConta">Conta *</label><select id="finLancConta">'+ops+"</select></div>"+
'<div class="campo"><label for="finLancCat">Categoria</label><select id="finLancCat">'+finOpcoesCat("",!1)+"</select></div>"+
'<div class="campo"><label for="finLancDom">Domínio</label><select id="finLancDom">'+finOpcoesDom("",!1)+"</select></div>"+
'<div class="campo"><label for="finLancObs">Observação</label><input id="finLancObs" type="text" autocomplete="off"></div>'+
'<div class="cad-acoes"><button class="btn-cad" data-acao="fin-lanc-ok">Lançar</button>'+
'<button class="btn-cad secundario" data-acao="fin-lanc-fechar">Cancelar</button></div>'+
...
// (<MEMO>texto e valido). Parser de XML quebra nisso, entao a leitura e por
// bloco e regex tolerante. O encoding costuma ser ISO-8859-1: le-se como
// ArrayBuffer e cai para windows-1252 quando o UTF-8 devolve o caractere de
// substituicao.
function finOfxTexto(buf){
var txt="";
try{txt=new TextDecoder("utf-8").decode(buf)}catch(err){txt=""}
if(!txt||txt.indexOf("�")>=0){
try{txt=new TextDecoder("windows-1252").decode(buf)}catch(err2){}}
return txt}
function finDesescapa(x){
return String(x||"").replace(/&amp;/gi,"&").replace(/&lt;/gi,"<").replace(/&gt;/gi,">")
.replace(/&#(\d+);/g,function(todo,cod){return String.fromCharCode(parseInt(cod,10))})}
function finOfxTag(bloco,tag){
var m=new RegExp("<"+tag+">([^<\\r\\n]*)","i").exec(String(bloco||""));
return m?finDesescapa(m[1]).trim():""}
function finOfxData(v){
var dig=String(v||"").replace(/[^0-9]/g,"");
return dig.length>=8?dig.slice(0,4)+"-"+dig.slice(4,6)+"-"+dig.slice(6,8):""}
function finOfxParse(txt){
var partes=String(txt||"").split(/<STMTTRN>/i),itens=[],i,bl,memo,nome,tipo,bruto;
for(i=1;i<partes.length;i++){
bl=partes[i].split(/<\/STMTTRN>/i)[0];
memo=finOfxTag(bl,"MEMO");nome=finOfxTag(bl,"NAME");tipo=finOfxTag(bl,"TRNTYPE");
bruto=finOfxTag(bl,"DTPOSTED");
itens.push({fitid:finOfxTag(bl,"FITID"),
data:bruto,
data_iso:finOfxData(bruto),
valor:finOfxTag(bl,"TRNAMT"),
descricao:memo||nome||tipo||"sem descrição"})}
var bal=/<LEDGERBAL>([\s\S]*?)(?:<\/LEDGERBAL>|<\/STMTRS>|$)/i.exec(String(txt||""));
return{itens:itens,
saldo:bal?finOfxTag(bal[1],"BALAMT"):"",
ini:finOfxData(finOfxTag(txt,"DTSTART")),
fim:finOfxData(finOfxTag(txt,"DTEND")),
banco:finOfxTag(txt,"ORG")||finOfxTag(txt,"BANKID")||""}}
async function finLerArquivo(file){
if(!file)return;
FIN_IMP=null;
var buf;
try{buf=await file.arrayBuffer()}
catch(err){FIN_PREVIA={erro:"Não consegui ler o arquivo: "+String(err&&err.message||err),itens:[]};return void renderFinanceiro(!0)}
var pr=finOfxParse(finOfxTexto(buf));
if(!pr.itens.length){
FIN_PREVIA={erro:"Não encontrei lançamentos nesse arquivo. Ele é um OFX de extrato?",itens:[]};
return void renderFinanceiro(!0)}
pr.arquivo=file;
pr.nome=file.name||"extrato.ofx";
FIN_PREVIA=pr;
renderFinanceiro(!0)}
function finPreviaTab(pr){
var vis=pr.itens.slice(0,8);
var linhas=vis.map(function(x){
var neg=parseFloat(String(x.valor).replace(",","."))<0;
return'<div class="fin-prev-lin"><span class="fin-prev-data">'+c(x.data_iso?fmtDiaCurto(x.data_iso):x.data)+"</span>"+
'<span class="fin-prev-desc">'+c(x.descricao)+"</span>"+
...
previa='<div class="estado erro"><strong>Não deu para usar esse arquivo.</strong>'+c(FIN_PREVIA.erro)+"</div>";
else if(FIN_PREVIA)
previa='<div class="fin-previa"><p class="cad-grupo">Prévia · nada foi gravado ainda</p>'+
'<div class="pitboard fin-placar">'+
'<div class="pb-celula"><div class="pb-rot">lançamentos</div><div class="pb-num">'+FIN_PREVIA.itens.length+'</div><div class="pb-pe">lidos do arquivo</div></div>'+
'<div class="pb-celula"><div class="pb-rot">período</div><div class="pb-num fin-num-p">'+c(FIN_PREVIA.ini?fmtDiaCurto(FIN_PREVIA.ini):"—")+" a "+c(FIN_PREVIA.fim?fmtDiaCurto(FIN_PREVIA.fim):"—")+'</div><div class="pb-pe">declarado no OFX</div></div>'+
'<div class="pb-celula"><div class="pb-rot">saldo final</div><div class="pb-num fin-num-p">'+c(FIN_PREVIA.saldo||"—")+'</div><div class="pb-pe">informado pelo banco</div></div>'+
'<div class="pb-celula"><div class="pb-rot">banco</div><div class="pb-num fin-num-p">'+c(FIN_PREVIA.banco||"—")+'</div><div class="pb-pe">'+c(FIN_PREVIA.nome||"")+"</div></div></div>"+
finPreviaTab(FIN_PREVIA)+
'<div class="campo"><label for="finImpConta">Conta *</label><select id="finImpConta">'+ops+"</select></div>"+
'<div class="cad-acoes"><button class="btn-cad" data-acao="fin-imp-ok">Confirmar importação</button>'+
'<button class="btn-cad secundario" data-acao="fin-imp-limpar">Descartar</button></div>'+
'<div class="fin-erro" id="finImpErro" role="alert"></div></div>';
return'<section class="fin-imp">'+res+
'<div class="fin-solta" id="finSolta"><svg class="fin-solta-ico" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 16V4.5M8 8.2l4-3.7 4 3.7" stroke-linecap="round" stroke-linejoin="round"></path><path d="M4 15v3.5A1.5 1.5 0 0 0 5.5 20h13a1.5 1.5 0 0 0 1.5-1.5V15" stroke-linecap="round"></path></svg>'+
'<div class="fin-solta-txt"><b>Arraste o arquivo .ofx aqui</b><span>ou escolha no seu aparelho. O arquivo é lido no navegador; nada sobe antes de você confirmar.</span></div>'+
'<input class="fin-solta-inp" id="finArq" type="file" accept=".ofx,application/x-ofx,application/xml,text/plain" aria-label="Arquivo OFX do extrato"></div>'+
previa+
'<p class="fin-rodape">O arquivo original vai para o balde privado <b>extrato</b>, para permitir reprocessar. Ele carrega o nome de quem pagou: nunca fica público.</p></section>'}
// ---- escrita ---------------------------------------------------------------
// A REGRA que manda em fin_classificar e a PRESENCA da chave, nao o valor:
// chave ausente nao mexe no campo, chave presente com null LIMPA. Por isso cada
// caminho monta o payload so com a chave que ele de fato quer tocar, e nenhum
// deles manda as duas por conveniencia.
async function finClassificar(pl,btn){
if(btn)btn.disabled=!0;
var r=await t.rpc("fin_classificar",{payload:pl});
if(btn)btn.disabled=!1;
if(r.error){
if(await pwSemSessao())return void pwSessaoCaiu();
return void I("Falha: "+r.error.message,!0)}
var d=r.data;
if(!d||!1===d.ok)return void I((d&&(d.erro||d.msg))||"Classificação recusada",!0);
// O aviso de natureza (saida gravada em categoria de entrada, por exemplo) NAO
// vira toast: toast some em 3,2s, e aviso que ninguem le nao e aviso. Ele fica
// na tela ate a proxima classificacao.
FIN_AVISO=d.aviso||"";
FIN_SEL={};
I(d.msg||(d.n||0)+" classificados");
renderFinanceiro(!0)}
function finLancErro(html){
var el=E("finLancErro");
if(el)el.innerHTML=html}
function finLancPayload(forcar){
var conta=E("finLancConta"),val=E("finLancValor"),sn=E("finLancSinal"),ds=E("finLancDesc");
var num=parseFloat(String(val?val.value:"").replace(",",".").trim());
if(!ds||!String(ds.value).trim())return{erro:"Escreva a descrição."};
if(!conta||!conta.value)return{erro:"Escolha a conta."};
if(isNaN(num)||num<=0)return{erro:"Informe um valor maior que zero. O sinal vem do campo Tipo."};
var pl={conta_id:conta.value,descricao:String(ds.value).trim(),
valor:("entrada"===(sn?sn.value:"saida")?"":"-")+num.toFixed(2)};
if(E("finLancData")&&E("finLancData").value)pl.data=E("finLancData").value;
if(E("finLancCat")&&E("finLancCat").value)pl.categoria_codigo=E("finLancCat").value;
if(E("finLancDom")&&E("finLancDom").value)pl.dominio=E("finLancDom").value;
if(E("finLancObs")&&String(E("finLancObs").value).trim())pl.observacao=String(E("finLancObs").value).trim();
if(forcar)pl.forcar=!0;
return{payload:pl}}
// A RPC recusa lancamento identico no mesmo dia e devolve `repetido`. Sem o
// botao abaixo o dono simplesmente NAO consegue registrar dois cafes iguais no
// mesmo dia, e a trava de duplicata vira trava de realidade.
async function finLancar(pl,btn){
if(btn)btn.disabled=!0;
var r=await t.rpc("fin_lancar",{payload:pl});
if(btn)btn.disabled=!1;
if(r.error){
if(await pwSemSessao())return void pwSessaoCaiu();
return void finLancErro(c("Falha: "+r.error.message))}
var d=r.data;
if(d&&d.repetido){
FIN_REPETIR=pl;
return void finLancErro(c(d.erro||"Já existe um lançamento igual nesse dia.")+
  ' <button class="btn-acao" data-acao="fin-lanc-forcar">Aconteceu duas vezes mesmo</button>'+
  (d.dica?'<span class="fin-dica">'+c(d.dica)+"</span>":""))}
if(!d||!1===d.ok)return void finLancErro(c((d&&(d.erro||d.msg))||"Lançamento recusado"));
FIN_REPETIR=null;FIN_LANC=!1;
I(d.msg||"Lançamento registrado");
renderFinanceiro(!0)}
function finNomeSeguro(nome){
var x=String(nome||"extrato.ofx").replace(/[^A-Za-z0-9._-]+/g,"_").replace(/^_+|_+$/g,"");
return x||"extrato.ofx"}
// Ordem sobe-depois-registra, o mesmo molde do anexo de NF. A diferenca e o
// tratamento da falha: aqui o upload e OPCIONAL, entao falha nele NAO aborta a
// importacao. Perder o extrato guardado e ruim; perder a importacao inteira,
// que e o trabalho que o dono acabou de conferir na previa, e pior. A tela DIZ
// que o arquivo nao ficou guardado, em vez de deixar isso silencioso.
async function finImportar(btn){
if(!FIN_PREVIA||!FIN_PREVIA.itens||!FIN_PREVIA.itens.length)
return void I("Nenhum lançamento para importar",!0);
var conta=E("finImpConta");
if(!conta||!conta.value)return void I("Escolha a conta antes de confirmar",!0);
if(btn)btn.disabled=!0;
var caminho="",aviso="",tn=await nfTenant();
if(!tn)aviso="O extrato não ficou guardado: a sessão não devolveu o tenant. A importação seguiu mesmo assim.";
else{
var alvo=tn+"/"+Date.now()+"-"+finNomeSeguro(FIN_PREVIA.nome);
var up=await t.storage.from("extrato").upload(alvo,FIN_PREVIA.arquivo,{contentType:"application/octet-stream",upsert:!1});
if(up&&up.error)aviso="O extrato não ficou guardado ("+(up.error.message||"falha no envio")+"). A importação seguiu mesmo assim.";
else caminho=alvo}
var pl={conta_id:conta.value,
itens:FIN_PREVIA.itens.map(function(x){
return{fitid:x.fitid,data:x.data,valor:x.valor,descricao:x.descricao}})};
if(caminho)pl.arquivo=caminho;
if(FIN_PREVIA.banco)pl.banco=FIN_PREVIA.banco;
if(FIN_PREVIA.ini)pl.periodo_ini=FIN_PREVIA.ini;
if(FIN_PREVIA.fim)pl.periodo_fim=FIN_PREVIA.fim;
if(FIN_PREVIA.saldo)pl.saldo_final_informado=FIN_PREVIA.saldo;
var r=await t.rpc("fin_importar_extrato",{payload:pl});
if(btn)btn.disabled=!1;
var alvoErro=E("finImpErro");
if(r.error){
if(await pwSemSessao())return void pwSessaoCaiu();
if(alvoErro)alvoErro.textContent="Falha: "+r.error.message;
return}
var d=r.data;
if(!d||!1===d.ok){
...
if(!ids.length)return void I("Selecione ao menos um lançamento",!0);
var lc=E("finLoteCat"),ld=E("finLoteDom"),lp={ids:ids},tocou=!1;
if(lc&&lc.value){tocou=!0;lp.categoria_codigo="__limpar"===lc.value?null:lc.value}
if(ld&&ld.value){tocou=!0;lp.dominio="__limpar"===ld.value?null:ld.value}
if(!tocou)return void I("Escolha a categoria ou o domínio que vai mudar",!0);
await finClassificar(lp,el);
return}
if("fin-cat"===acao){
var vc=el.value||"",ac=el.getAttribute("data-atual")||"";
if(vc===ac)return;
await finClassificar({ids:[id],categoria_codigo:vc||null},el);
return}
if("fin-dom-lin"===acao){
var vd=el.value||"",ad=el.getAttribute("data-atual")||"";
if(vd===ad)return;
await finClassificar({ids:[id],dominio:vd||null},el);
return}
if("fin-lanc-abrir"===acao){FIN_LANC=!0;FIN_REPETIR=null;return void renderFinanceiro(!0)}
if("fin-lanc-fechar"===acao){FIN_LANC=!1;FIN_REPETIR=null;return void renderFinanceiro(!0)}
if("fin-lanc-ok"===acao){
var mp=finLancPayload(!1);
if(mp.erro)return void finLancErro(c(mp.erro));
finLancErro("");
await finLancar(mp.payload,el);
return}
if("fin-lanc-forcar"===acao){
if(!FIN_REPETIR)return;
var rep=FIN_REPETIR;
rep.forcar=!0;
finLancErro("");
...
// O input de arquivo tambem fica aqui, e nao em data-acao: o delegado escuta
// click, e um click no proprio input (para trocar de arquivo) reprocessaria o
// arquivo anterior e fecharia a janela de selecao no meio.
function finLigar(){
var zona=E("finSolta"),inp=E("finArq");
if(inp)inp.addEventListener("change",function(){
if(inp.files&&inp.files[0])finLerArquivo(inp.files[0])});
if(!zona)return;
zona.addEventListener("dragover",function(ev){
ev.preventDefault();
if(ev.dataTransfer)ev.dataTransfer.dropEffect="copy";
zona.className="fin-solta alvo"});
zona.addEventListener("dragleave",function(){zona.className="fin-solta"});
zona.addEventListener("drop",function(ev){
ev.preventDefault();
zona.className="fin-solta";
var f=ev.dataTransfer&&ev.dataTransfer.files&&ev.dataTransfer.files[0];
if(f)finLerArquivo(f)})}
// fin_config e chamada UMA vez por sessao: conta e categoria sao config, nao
// dado do dia, e reler a cada troca de mes seria gastar rede para receber a
// mesma resposta.
async function renderFinanceiro(sil){
var e=E("lista");
if(!e)return;
if(!sil)e.innerHTML='<div class="estado carregando">Lendo o financeiro…</div>';
if(!FIN_CFG){
var rc=await t.rpc("fin_config",{});
if(rc.error)return void(e.innerHTML=estadoErro("o financeiro",rc.error.message));
var dc=rc.data;
...
// A MESMA linha serve os dois lugares onde a lista aparece, e o que muda e o
// papel do chip. No relatorio ele e um BOTAO que despacha; no painel de
// Motoboys (17/08/2026) nao ha venda aberta, entao despachar nao existe e o
// chip vira texto. Chip clicavel que nao faz nada e pior que chip parado: o
// dono toca, nada acontece, e isso e indistinguivel de defeito.
function motoLinha(m,ger){
var corpo="<strong>"+c(m.nome||"sem nome")+"</strong><span>"+(m.whatsapp?c(f(m.whatsapp)):"sem WhatsApp")+"</span>";
return'<div class="moto-linha">'
+(ger?'<div class="moto-chip fixo">'+corpo+"</div>"
:'<button type="button" class="moto-chip" data-acao="ent-moto-enviar" data-id="'+c(m.id)+'">'+corpo+"</button>")
+'<button type="button" class="moto-rm" data-acao="ent-moto-rm" data-id="'+c(m.id)+'" aria-label="Tirar da lista">tirar</button></div>'}
// Pinta os DOIS containers de uma vez. Uma funcao so porque a lista e uma so:
// salvar no painel de Motoboys com o relatorio aberto atras tem que atualizar os
// dois, senao a tela passa a mostrar duas versoes da mesma tabela.
function motoPintarEm(id,ger){
var cx=E(id);
if(!cx)return;
cx.innerHTML=motoboysData.length?motoboysData.map(function(m){return motoLinha(m,ger)}).join("")
...
motoPintarEm("peMotoLista",!1);
motoPintarEm("pmLista",!0)}
...
// Os ids dos campos entram por parametro porque a MESMA gravacao e disparada de
// dois lugares (o rodape do relatorio e o painel de Motoboys). Um segundo
// salvarMotoboy seria a segunda regra que envelhece sozinha: a validacao de nome
// vazio ja mora aqui, e duplicar isso e como duas telas passam a discordar.
var MOTO_CX_ENT={nome:"peMotoboy",tel:"peMotoWhats",erro:"peErro",limpa:!1};
async function salvarMotoboyUI(btn,cx){
cx=cx||MOTO_CX_ENT;
var err=function(m){if(E(cx.erro))E(cx.erro).textContent=m};
var nome=entVal(cx.nome),tel=entVal(cx.tel);
if(!nome){err("Digite o nome do motoboy antes de salvar na lista.");return}
if(btn)btn.disabled=!0;
...
err("");
// So o painel de Motoboys limpa os campos. No relatorio eles sao o motoboy DA
// CORRIDA: apagar ali tiraria da mensagem quem acabou de ser escolhido.
if(cx.limpa){if(E(cx.nome))E(cx.nome).value="";if(E(cx.tel))E(cx.tel).value="";if(E(cx.nome))E(cx.nome).focus()}
...
// ---- Painel de Motoboys, porta propria (17/08/2026) -------------------------
// Ate aqui a lista so existia dentro do relatorio de entrega, e o dono reportou
// que "a adicao de motoboy na lista esta perdida, aparece apenas quando clico em
// relatorio". Era verdade: sem uma venda com entrega aberta nao havia caminho
// nenhum para ver ou cadastrar motoboy. Mesma tabela, mesmas RPCs, mesma pintura.
// O painel NAO despacha: despachar exige uma venda para saber o que entregar.
function abrirPainelMotoboys(){
if(E("pmErro"))E("pmErro").textContent="";
if(E("pmNome"))E("pmNome").value="";
if(E("pmWhats"))E("pmWhats").value="";
if(E("painelMotoboys"))E("painelMotoboys").className="painel-cadastro";
// O botao que abre este painel vive no topo da aba, mas a aba rola: sem isto o
// painel abriria fora da vista e o toque pareceria nao ter feito nada. Mesmo
// cinto do painel de venda (medido em 15/08: 4.415px de distancia).
pwRolarPara("painelMotoboys");
pintarMotoboys();
carregarMotoboys().then(pintarMotoboys);
if(E("pmNome"))E("pmNome").focus()}
function fecharPainelMotoboys(){
if(E("painelMotoboys"))E("painelMotoboys").className="painel-cadastro oculto"}
// Delegado proprio: este painel vive fora de #lista, entao nao pega o A().
function motoPainelClick(ev){
...
if("pm-salvar"===o){ev.preventDefault();salvarMotoboyUI(el,{nome:"pmNome",tel:"pmWhats",erro:"pmErro",limpa:!0});return}}
...
// Mesmo cinto do "+ Nova venda" (v58): o Abrir relatorio mora DENTRO do card da
// venda, no meio da lista, e o painel nasce no topo da pagina. Sem esta rolagem
// o dono toca e a tela nao muda, que e indistinguivel de "o botao nao funciona".
// Foi exatamente assim que ele reportou, nas duas vezes.
pwRolarPara("painelEntrega");
...
//   - status pre_venda TAMBEM NAO CONTA (dono, 17/08/2026). Ate aqui ela
//     entrava na soma e era declarada no cabecalho; agora sai de todo numero
//     de dinheiro e o cabecalho declara o que FICOU DE FORA. Dinheiro que
//     ainda nao fechou somado ao que fechou nao e faturamento, e previsao.
// A mesma migration `painel_metricas_so_concluida` levou o criterio novo para
// a RPC no mesmo dia: mudar so este lado faria a aba Metricas e a aba Vendas
// darem dois numeros para a mesma palavra.
...
// Qual card esta aberto em detalhe: "" (nenhum), "fat" ou "luc". UM por vez, de
// proposito: dois blocos abertos empurram o grafico para fora da tela no
// celular, e a pergunta "de quem e esse numero" e sempre sobre UM numero.
var vgDet="";
// UMA funcao decide o que vale dinheiro, e os quatro pontos de soma da tela
// (placar, grafico por mes, recorte de origem e o Dashboard) chamam ela. Antes o
// criterio era a mesma condicao copiada em quatro `if`: mudar de ideia obrigava
// a achar as quatro, e esquecer uma so faz dois numeros da mesma tela
// discordarem sem ninguem notar.
// E `status`, nunca `etapa`: status responde "vale dinheiro?", etapa responde
// "onde esta?" (v61). Venda concluida que ainda nao saiu para entrega ja e
// faturamento; pre-venda entregue ainda nao e.
function vgConta(v){return "concluida"===v.status}
...
var r={n:0,pre:0,preFat:0,fat:0,luc:0,custo:0,frete:0,taxas:0,meses:[]},mapa={},i,v,d,ym,m,fat,luc;
for(i=0;i<linhas.length;i++){
v=linhas[i];
if("cancelada"===v.status)continue;
d=vgDataDe(v);
if(!d||d<lim.ini||d>lim.fim)continue;
// A pre-venda sai ANTES de qualquer soma, inclusive de `n`. Tirar do
// faturamento e deixar no contador seria pior que nao mexer: o ticket medio
// passaria a dividir o dinheiro de quem fechou pelo numero de quem nao
// fechou, e o erro ficaria escondido numa celula que ninguem confere.
// Ela nao some: vira `pre`/`preFat`, que o cabecalho declara em voz alta.
if(!vgConta(v)){r.pre++;r.preFat+=Number(v.valor_venda)||0;continue}
...
'<div class="pb-pe">'+c(a.n?brlV(a.luc/a.n)+" por venda":"nada fechado ainda")+"</div>"+vgDetBtn("luc",a)+"</div>"+
...
(det||"")+"</div>"}
// ---- Detalhe do numero (18/08/2026): de quem e esse faturamento -------------
// Pedido do dono no mesmo dia em que a pre-venda saiu da soma, e os dois pedidos
// sao o mesmo pedido: numero somado por criterio que a tela nao mostra e numero
// em que so se acredita. Aqui a soma se abre venda a venda, e a ultima linha do
// bloco RESOMA na tela, para o card e o detalhe nunca poderem discordar sem que
// se veja.
// Zero rede: sai de vendasData, que a aba ja carregou (regra do v51 secao 4).
// A MESMA porta de entrada do vgAgregar (mesmo descarte de cancelada, mesma
// janela, mesmo vgConta). Se o detalhe tivesse condicao propria, ele explicaria
// um numero que nao e o do card, que e o pior defeito possivel numa superficie
// de auditoria: parece conferencia e e uma segunda conta.
// Mais nova primeiro (invariante 6).
function vgDetSeparar(linhas,lim){
var ok=[],pre=[],i,v,d,ord=function(x,y){var a=vgDataDe(x),b=vgDataDe(y);return a<b?1:a>b?-1:0};
for(i=0;i<linhas.length;i++){
v=linhas[i];
if("cancelada"===v.status)continue;
d=vgDataDe(v);
if(!d||d<lim.ini||d>lim.fim)continue;
(vgConta(v)?ok:pre).push(v)}
ok.sort(ord);pre.sort(ord);
return{ok:ok,pre:pre}}
function vgDetQuem(v){
return(v.modelo_rotulo||v.modelo_texto||"sem modelo")+(v.capacidade?" "+v.capacidade:"")}
// Uma linha = uma venda. O numero da direita e SEMPRE o do card aberto, nunca os
// dois: coluna que muda de significado conforme a linha e como duas leituras da
// mesma tela passam a discordar.
// O modo "pre" existe para a pre-venda nao emprestar o vocabulario de quem
// fechou: ela mostra o valor previsto e diz, na propria linha, que esta fora.
function vgDetLin(v,modo,tot){
var luc="luc"===modo,fat=Number(v.valor_venda)||0;
var val=luc?Number(v.lucro)||0:fat;
var pe=luc?"margem "+vgPct1(val,fat)+" \u00b7 venda "+brlV(fat)
:"pre"===modo?"pr\u00e9-venda, fora da soma"
:tot>0?vgPct1(val,tot)+" do faturamento":"\u2014";
return'<div class="vg-det-lin"><div class="vg-det-quem">'+
'<span class="vg-det-quando">'+c(vgDataBr(vgDataDe(v)))+"</span>"+
"<strong>"+c(vgDetQuem(v))+"</strong>"+
'<span class="vg-det-cli">'+c(v.cliente_nome||"sem cliente")+
(v.venda_code?" \u00b7 "+c(v.venda_code):"")+"</span></div>"+
'<div class="vg-det-cifra"><div class="vg-det-val'+(val<0?" neg":"")+'">'+c(brlV(val))+"</div>"+
'<div class="vg-det-pe">'+c(pe)+"</div></div></div>"}
// O botao so existe quando ha o que abrir. Numa janela sem venda nenhuma ele
// abriria um bloco vazio, e affordance que nao entrega nada e ruido.
function vgDetBtn(k,a){
if(!a.n&&!a.pre)return"";
var ab=vgDet===k;
return'<button type="button" class="vg-det-btn" data-acao="vg-detalhe" data-id="'+c(k)+
'" aria-expanded="'+(ab?"true":"false")+'">'+(ab?"fechar detalhe":"detalhar")+"</button>"}
// O bloco mora DENTRO da faixa de valores (grid-column 1/-1), colado nos cards,
// e nao depois dos graficos: no celular a resposta tem que nascer onde a
// pergunta foi feita, senao ela abre fora da vista e o toque parece nao ter
// funcionado (mesma licao do pwRolarPara).
function vgDetBloco(linhas,lim,a){
var modo="luc"===vgDet?"luc":"fat";
var s=vgDetSeparar(linhas,lim),i,corpo="",fora="";
var som="luc"===modo?a.luc:a.fat;
for(i=0;i<s.ok.length;i++)corpo+=vgDetLin(s.ok[i],modo,a.fat);
if(s.pre.length){
fora='<p class="vg-det-fora">'+c(s.pre.length+" pr\u00e9-venda"+(1===s.pre.length?"":"s")+
" fora da soma ("+brlV(a.preFat)+")")+
"<span>Dinheiro que ainda nao fechou. Entra na soma quando a venda for concluida.</span></p>";
for(i=0;i<s.pre.length;i++)fora+=vgDetLin(s.pre[i],"pre",0)}
return'<div class="vg-det" data-cel="vg-detalhe" data-modo="'+c(modo)+'">'+
'<div class="vg-det-cab"><div class="vg-det-tit">'+("luc"===modo?"lucro":"faturamento")+
', venda a venda</div>'+
'<div class="vg-det-rec">'+c(s.ok.length+(1===s.ok.length?" venda conclu\u00edda":" vendas conclu\u00eddas")+
" \u00b7 de "+vgDataBr(lim.ini)+" a "+vgDataBr(lim.fim))+"</div>"+
'<button type="button" class="vg-det-fechar" data-acao="vg-detalhe" data-id="">Fechar</button></div>'+
(corpo||'<p class="vg-det-vazio">Nenhuma venda conclu\u00edda nesta janela.</p>')+
(s.ok.length?'<div class="vg-det-soma"><span>soma das linhas</span><b'+(som<0?' class="neg"':"")+
">"+c(brlV(som))+"</b></div>":"")+
fora+
'<p class="vg-det-nota">Cancelada e arquivada n\u00e3o entram aqui, nem na soma.</p>'+
"</div>"}
...
// Ela mudou de lado em 17/08/2026: dizia "inclui 1 pre-venda" e agora diz
// quanto ficou DE FORA, com o valor. Numero excluido em silencio e a mesma
// omissao de antes, so que na direcao contraria: o dono veria R$ 22.120 sem
// saber que existem R$ 6.340 em transito.
...
var rec="de "+vgDataBr(lim.ini)+" a "+vgDataBr(lim.fim)+" · "+a.n+
(1===a.n?" venda concluída":" vendas concluídas")+
(a.pre?" · "+a.pre+" pré-venda"+(1===a.pre?"":"s")+" fora da soma ("+brlV(a.preFat)+")":"");
...
// `a` entrou aqui em 17/08/2026: janela onde SO ha pre-venda tem a.n === 0 e
// caia neste aviso dizendo "nenhuma venda", com duas vendas na tela logo abaixo.
// Vazio que contradiz a lista e pior que vazio: o aviso agora nomeia o motivo.
function vgVazio(a){
var pre=a&&a.pre?" "+a.pre+" pré-venda"+(1===a.pre?"":"s")+" nesta janela não "+
(1===a.pre?"entra":"entram")+" na soma ("+brlV(a.preFat)+").":"";
return'<div class="vg-vazio"><strong>Nenhuma venda concluída nesta janela.</strong>'+c(pre)+' '+
("tudo"===vgJanela?"A base ainda não tem venda concluída."
...
// Mesma janela e MESMO criterio de faturamento do painel de cima (so concluida,
// cancelada e pre-venda fora, arquivada ja filtrada pela v_venda): um segundo
// criterio de faturamento e como um sistema passa a discordar de si mesmo.
// Por isso o filtro aqui e a chamada a vgConta, nao uma copia da condicao.
...
'<div class="vg-corpo">'+vgValores(a,vgDet?vgDetBloco(linhas,lim,a):"")+
'<div class="vg-graf g-meses">'+(a.n?vgMeses(a):vgVazio(a))+"</div>"+
...
out.push(i[j])}
return out}
// Mesmo vgConta do painel de Vendas: os cards de dinheiro daqui (faturamento,
// lucro, margem, ticket, canal, condicao, aparelhos) sao o MESMO conceito da
// aba ao lado. O card de status e a unica coisa que continua lendo vendasData
// cru, e tem que continuar: ele existe justamente para contar quantas estao em
// cada situacao, pre-venda e cancelada incluidas.
...
// O Motoboys entra como acao SECUNDARIA ao lado do Nova venda: e manutencao de
// cadastro, nao o ato principal da aba. Fica no topo pelo mesmo motivo do Nova
// venda, que o comentario abaixo mede: botao que abre painel no topo mora no topo.
var topo='<div class="venda-topo"><span class="venda-topo-acoes">'+
'<button class="btn-cad" data-acao="nova-venda">+ Nova venda</button>'+
'<button class="btn-cad secundario" data-acao="venda-motoboys">Motoboys</button></span>'+
'<span class="venda-cont">'+vendasData.length+(1===vendasData.length?" venda":" vendas")+arq+"</span></div>";
...
return!0}
if("venda-entrega"===o){abrirPainelEntrega(id);return!0}
if("venda-motoboys"===o){abrirPainelMotoboys();return!0}if("venda-desarquivar"===o){desarquivarVenda(id);return!0}
...
// Alternar, nao acumular: tocar no card ja aberto fecha, e o Fechar manda
// data-id vazio. Repinta a aba pelo mesmo caminho do vg-janela, sem releitura
// de rede (vendasData nao mudou).
if("vg-detalhe"===o){vgDet=id&&id!==vgDet?id:"";vgTipEsconder();pintarVendas();return!0}
...
// Nome da loja vem do banco, nunca do HTML. A policy p_tenant_select ja permite
// id = fn_tenant_atual(), entao a leitura sai com o token do proprio usuario, sem
// RPC nova. Degradacao: falhou a leitura, fica so "Pit Wall" e o data-loja vazio.
// Nunca escrever nome de loja fixo aqui: e isso que fazia o parceiro abrir o
// sistema dele lendo o nome da loja do dono.
async function pwLoja(){
try{
var r=await t.from("tenant").select("nome").maybeSingle();
var n=r&&r.data&&r.data.nome?String(r.data.nome).trim():"";
if(!n)return;
document.title="Pit Wall · "+n;
var els=document.querySelectorAll("[data-loja]");
