...
- anterior: `handoff_calculadora_pitwall_v9.md` (11/09/2026, **o laco de aprendizado
  ...
  **D14, decidida e REVISADA na mesma sessao:** a primeira resposta (*"a"*, padrao
  silencioso por fornecedor) caiu. Vale: *"na verdade, pergunte quando nao houver
  condição descrita"*. Uma pergunta por fornecedor, por lista, com a resposta
  anterior pre-selecionada como sugestao; "o perfil desempata, nunca decide" fica
  sem excecao. A parte de leitor vai junto com o `2.4a zero`.
  **Achado novo, classe preco errado:** o banner `LACRADOS E SEMINOVOS` grava tudo
  como `Lacrado`, calado. Proposta ao dono: `Seminovo` junto de outra condicao vira
  pergunta; `CPO` + `Lacrado` segue `CPO`.
