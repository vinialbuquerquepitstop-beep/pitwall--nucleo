-- ─────────────────────────────────────────────────────────────────────────────────
-- calc_parse_v2, quarta rodada: o papel `quebra` sai, por decisao do dono.
--
-- Nao e mais um remendo: e a troca da regra que causou os tres anteriores.
--
-- ── O que a medicao mostrou, rodada a rodada ────────────────────────────────
--
-- `quebra` dizia: "linha que eu nao reconheco = o fornecedor mudou". Toda vez
-- que uma linha real caia nele, TUDO abaixo perdia o fornecedor e virava
-- duvidoso. Medido nesta sessao, em tres listas diferentes:
--
--   '🟠laranja (eSIM)'         ->  25 linhas derrubadas   (MP)
--   '🍎 iPhone Air  – 256GB'    ->  14 linhas derrubadas   (BR10)
--   '* ⁠anatel 🇧🇷'              ->  28 linhas derrubadas   (MP)
--
-- As duas primeiras foram consertadas por classificacao nova (`cor`,
-- `modelo_desconhecido`). A terceira mostrou o padrao: numa lista de WhatsApp,
-- linha nao reconhecida e o caso COMUM, nao a excecao. Qualificador, decoracao,
-- aviso, endereco, saudacao. Cada uma destruia o resto da lista, e a proxima
-- lista traria uma quarta.
--
-- ── A regra nova ────────────────────────────────────────────────────────────
--
-- O fornecedor vale do cabecalho DELE ate o proximo cabecalho de fornecedor
-- RECONHECIDO. Linha desconhecida deixa de significar troca de fornecedor.
--
-- ── O que se perde, declarado ───────────────────────────────────────────────
--
-- A trava que sai e real e estava escrita com todas as letras no v1: preco de
-- fornecedor nao identificado nao pode herdar o nome do fornecedor anterior,
-- porque preco certo no fornecedor errado passa no validador, passa no diff, e
-- so aparece quando alguem compra pelo custo de outra loja.
--
-- Isso continua verdade. O que mudou e ONDE a defesa mora: sai do parser e vai
-- para a tela. O parser ja devolve `cabecalhos` com o que reconheceu, e o
-- wizard nao grava carga sem diff aprovado por gente (restricao do PROCESSO).
-- Uma lista com um terceiro fornecedor fora do catalogo passa a atribuir os
-- precos dele ao fornecedor anterior, e quem tem que pegar isso e o dono, na
-- conferencia, nao a heuristica.
--
-- Trocar uma trava automatica por conferencia humana e escolha com custo real.
-- Foi levada ao dono com os tres numeros acima e ele escolheu a troca,
-- conscientemente, em 09/09/2026.
--
-- ── O que NAO muda ──────────────────────────────────────────────────────────
--
-- `modelo_desconhecido` continua zerando o modelo herdado, e essa e a trava que
-- importa de verdade: sem ela, os precos abaixo de um cabecalho que o catalogo
-- nao conhece herdariam o modelo do bloco ANTERIOR, e o preco de um aparelho
-- seria gravado no nome de outro. Pendencia e barata; atribuicao errada, nao.
-- O papel `quebra` segue existindo como rotulo do que nao casou nada; ele so
-- deixou de interromper o bloco do fornecedor.
-- ─────────────────────────────────────────────────────────────────────────────────
