-- ─────────────────────────────────────────────────────────────────────────────
-- calc_parse_v2, terceira rodada. Duas causas medidas contra as listas reais,
-- as duas com o MESMO efeito: derrubar o resto da lista inteira.
--
-- Medido em 09/09/2026, depois da rodada anterior:
--
--   BR10, primeira metade   29 de 29   100,0%   <- o desenho funciona
--   BR10, segunda metade     8 de 31    25,8%
--   MP IMPORTS, iPhones      0 de 25     0,0%
--
-- A primeira metade do BR10 fechando em 100% prova que heranca de modelo,
-- pareamento de cor e descarte de bloco estao certos. O que derruba as outras
-- duas nao e o casamento: e o papel `quebra` demolindo o bloco do fornecedor a
-- partir de UMA linha mal classificada, e todas as linhas seguintes caindo em
-- `duvidoso` por ficarem sem fornecedor.
--
-- ── Causa 1: qualificador tecnico entre parenteses (mata o MP inteiro) ──────
--
-- A regra `so_cor` da rodada anterior tira as cores conhecidas da linha e ve se
-- sobrou palavra. No MP a cor vem com o qualificador junto:
--
--   '🔵azul'            -> sobra '🔵 '         -> e cor   ✔
--   '🟠laranja (eSIM)'  -> sobra '🟠  (esim)'  -> NAO e cor  ✘  -> quebra
--
-- `(eSIM)`, `(esim)`, `(Anatel 🇧🇷)` sao qualificadores de aparelho, nunca parte
-- do nome da cor. A PRIMEIRA linha de cor do MP ja e `🟠laranja (eSIM)`, entao a
-- lista morria no primeiro produto e as 25 linhas seguintes viravam duvidoso.
--
-- Conserto: o teste `so_cor` passa a ignorar o que esta entre parenteses. Nao e
-- lista de palavras: e a observacao de que parentese nessas listas carrega
-- qualificador, e a condicao entre parenteses (`(CPO)`) continua sendo lida
-- normalmente por `cond_linha`, que roda sobre o texto inteiro e nao por aqui.
--
-- ── Causa 2: cabecalho de produto sem alias demolia o resto da lista ────────
--
-- Na segunda metade do BR10, `🍎 iPhone Air  – 256GB` nao casa modelo (o
-- catalogo tem `iPhone 17 Air 256GB`, e falta o apelido). Sem casar, ele caia em
-- `quebra`; como ja havia precos desde o cabecalho do fornecedor, o `quebra`
-- era efetivo e RESETAVA o fornecedor. Resultado medido na pendencia:
--
--   'iPhone Air  – 256GB'  -> 14 linhas orfas de fornecedor
--   'Ultra 3 – 49mm'       ->  3 linhas orfas
--
-- 17 das 31 linhas perdidas por DOIS cabecalhos. Um modelo que o catalogo ainda
-- nao conhece nao pode custar a lista inteira: ele devia custar o proprio bloco.
--
-- Conserto: papel novo `modelo_desconhecido`, para linha sem preco que tem cara
-- de produto (capacidade, tamanho em mm ou polegada declarados) e mesmo assim
-- nao casou modelo. Ele:
--   - NAO quebra o bloco do fornecedor;
--   - ZERA o modelo herdado, e isso e o ponto que importa. Sem zerar, os precos
--     abaixo de `iPhone Air 256GB` herdariam `iPhone 17 256GB` do bloco anterior
--     e gravariam preco de um aparelho no nome de OUTRO. Pendencia e barata;
--     atribuicao errada, nao;
--   - vira pendencia de modelo, que e onde o dono ensina o apelido ao catalogo.
--
-- O teste de "cara de produto" e capacidade/mm/polegada, nao familia
-- (`iphone`, `apple`...): `⚠️ GARANTIA DE 1 ANO DIRETO COM APPLE ⚠️` contem
-- `apple` e nao e produto nenhum. Falso positivo aqui custa uma pendencia; falso
-- negativo custa a lista, entao o teste e o especifico.
--
-- ── Causa 3, junto: cor decorada tambem nas cores soltas ───────────────────
--
-- A trava de `verde menta` / `azul titanio` (nunca inventar hex para cor que o
-- catalogo so conhece pela metade) so rodava sobre linha COM preco. As cores
-- soltas da rodada anterior entravam sem passar por ela. Agora passam, com a
-- mesma remocao de parenteses: `laranja (esim)` nao e cor decorada, mas
-- `verde menta` continua sendo.
-- ─────────────────────────────────────────────────────────────────────────────
