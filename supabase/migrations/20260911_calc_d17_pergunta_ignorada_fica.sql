-- 20260911_calc_d17_pergunta_ignorada_fica.sql
-- Fecha a D17 (a) de verdade. A migration anterior
-- (`20260911_calc_parse_pergunta_de_fornecedor_fica.sql`) passou nas 104
-- assercoes e a `bandeira` REPROVOU assim mesmo, por um caso que nenhuma delas
-- cobria, e ela estava certa: o criterio e o texto que o dono decidiu, nao a
-- amostra que a prova escolheu.
--
-- O CASO, medido pela `bandeira` (classe PRECO ERRADO)
-- Duas mensagens do mesmo fornecedor: `XPTO CELL IMPORTS` com o 16 256GB a
-- 4.100, e `*Xpto  Cell  Imports*` com o mesmo modelo a 4.900. O dono IGNORA a
-- primeira ("deixe estas linhas fora desta lista") e cria o fornecedor pela
-- segunda. O blob saia com 4.100: o preco da mensagem ignorada.
-- Causa: o leitor reconhece fornecedor pelo NOME (`position(calc_norm(nome) in
-- linha)`), e `XPTO CELL IMPORTS` contem `xpto cell imports`. O papel
-- `forn_aberto` vinha DEPOIS de `forn` no `case`, entao um nome reconhecido
-- vencia a pergunta ignorada, e a D17 (a) nao valia para ela.
--
-- O CONSERTO, e por que ele cabe na D17 (a) sem decisao nova
-- `forn_aberto` passa a vir ANTES de `forn`. A (a) decidida pelo dono diz: a
-- pergunta ja feita fica de pe. Agora ela fica de pe ate o dono responder ELA,
-- e nao so contra o fornecedor de cima. O custo e no maximo um clique a mais
-- (confirmar uma grafia que o nome ja reconheceria); o ganho e nunca entrar
-- preco que o dono mandou deixar fora. A primeira leitura (`calc_carga_abrir`)
-- segue com a lista vazia, entao a D10 e a D13 continuam exatamente iguais.
--
-- JUNTO, um defeito do `criar` que a `bandeira` achou no mesmo passe
-- `criar` sobre a PROPRIA pergunta que o dono tinha ignorado nao a incluia:
-- apelido e marcacao pegavam so `decisao is null`. A pendencia seguia dizendo
-- `ignorar` com o efeito de `criar` (o historico mentia, invariante 6), e o
-- `grafias` voltava 2 ou 0. Agora a pergunta respondida entra sempre; a IRMA
-- ignorada continua de fora, de proposito.
-- E o comentario do `criar` que citava a "assercao C9": ela se chama K10.
--
-- Mesmas assinaturas: `create or replace` PRESERVA a ACL (medido em 11/09/2026).
-- O REVOKE/GRANT do fim e restricao global 5, nao correcao.
--
-- Gerada por script (gera_d17b.py, no scratchpad da sessao) a partir dos corpos
-- VIVOS, conferidos por md5 antes de mexer (leitor 2f6addaf..., criar
-- 4f3b1f4a...). Quatro trocas, cada uma tinha que achar o trecho exatamente uma
-- vez.

begin;

-- ── 1. O leitor: mesma assinatura, `create or replace` preserva a ACL ─────────
...
begin
  ...
  cab2 as materialized (
    select c.*,
           ...
           case when btrim(c.txt) = any(coalesce(p_forn_abertos, '{}'::text[]))
                                              then 'forn_aberto'
...
declare
  ...
  -- junto: a assercao K10 da prova compara as duas e reprova se divergirem.
...
begin
  ...
  -- A pergunta RESPONDIDA entra sempre, qualquer que seja a decisao anterior
  -- dela (`criar` sobre uma pergunta que ele tinha ignorado e resposta nova, e
  -- o historico tem que dizer `criar`, nao `ignorar`: invariante 6). A IRMA que
  -- ele ignorou NAO entra: "deixar estas linhas fora desta lista" continua
  -- valendo ate ele responder aquela pergunta, e o leitor a mantem fora (D17).
...
-- ── 3. Grants explicitos (restricao global 5), mesmo com o replace preservando
