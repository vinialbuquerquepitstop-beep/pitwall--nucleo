-- 20260911_calc_d17_forn_aberto_sem_fornecedor.sql
-- Terceira e ultima peca da D17 (a). A anterior
-- (`20260911_calc_d17_pergunta_ignorada_fica.sql`) DIZIA fechar o caso da
-- pergunta ignorada e nao fechava: o `base`, ao aplicar, rodou a fumaca e o blob
-- saiu com o mesmo 4.100 de antes. O cabecalho daquela migration afirma um
-- conserto que nao aconteceu; ela fica como esta porque ja foi aplicada, e o
-- registro honesto e este.
--
-- O ERRO, da Torre
-- Pos `forn_aberto` antes de `forn` no `case` e tratou isso como conserto. Mas
-- o `case` so escreve o ROTULO `papel`. O fornecedor da linha vem de `h.forn`
-- (CTE `linha`), calculado em `cab` pelo casamento por NOME, que nao olha o
-- papel. Com `XPTO Cell Imports` no catalogo, o cabecalho ignorado
-- `XPTO CELL IMPORTS` virava `forn_aberto` E continuava com fornecedor, entao as
-- linhas dele casavam e o menor preco ganhava. A primeira D17 (K13) funcionou
-- por acaso de dado: la o cabecalho do XPTO nao casava nome nenhum, entao
-- `h.forn` ja era nulo com ou sem o papel novo.
--
-- O CONSERTO
-- Uma linha: `hdr_forn` e nulo quando o cabecalho e `forn_aberto`. A linha cai em
-- duvidoso, a pergunta volta com o mesmo `hdr_txt` (a linha de `calc_pendencia`
-- ja existe como `ignorar`, e o `on conflict do nothing` nao duplica), e o
-- comentario da CTE `marc` ("ali o fornecedor e nulo") passa a ser verdade.
-- O `cabs` ja filtrava por `papel = 'forn'`; nenhum outro ponto do leitor le
-- `forn` de cabecalho (conferido por busca no corpo vivo).
-- Na abertura a lista e vazia: nenhum cabecalho e `forn_aberto`, entao a
-- primeira leitura nao muda.
--
-- Gerada por script (gera_d17c.py, no scratchpad da sessao) a partir do corpo
-- VIVO do leitor, conferido por md5 antes de mexer (28c938c1...). Uma troca.
...
begin
  ...
  linha as materialized (
    select m.ln, m.txt, m.nz, m.nzq, m.preco, m.pendura, m.cond_linha, m.cond_mista,
           ...
           case when h.papel = 'forn_aberto' then null else h.forn end as hdr_forn,
           h.txt  as hdr_txt,
...
-- Restricao global 5: mesmo com o `replace` preservando a ACL. Sem grant: e
-- `privado`, a barreira de papel mora nas RPCs de `public`.
