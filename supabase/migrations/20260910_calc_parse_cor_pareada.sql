-- ─────────────────────────────────────────────────────────────────────────────────
-- calc_parse_v2: a cor solta pareia com o preco POR ORDEM, nao so por distancia.
--
-- Achado em 10/09/2026 ao escrever `ferramentas/prova_calc_parse.sql` para
-- cobrir o v2 (item 1 da secao 5 do handoff v6). O handoff afirmava tres
-- formatos de cor cobertos; ao medir, um deles devolvia PRECO ERRADO.
--
-- ── O defeito, medido e isolado ────────────────────────────────────────────
--
--   *🍎 iPhone 14 Pro – 256GB*
--   🔒 Lacrado
--   🟣 Roxo
...
--   💵 *R$ 4.520,00*
--
--   antes:  Roxo = 4480 (certo)   Gold = 4480 (ERRADO)   4.520 -> orfa
--   depois: Roxo = 4480           Gold = 4520
--
-- Causa: `cor_assoc` escolhia o preco nu pelo `order by abs(p.ln - cs.cor_ln),
-- p.ln`. Uma cor entre dois precos empata em distancia 1 com os dois, e o `p.ln`
-- desempatava para CIMA, ou seja, para o preco que ja era da cor anterior.
--
-- No layout "cor DEPOIS do preco" o mesmo desempate acerta. Metade dos casos
-- dava certo, e foi isso que segurou o defeito no ar.
--
-- ── Por que so a prova pega ────────────────────────────────────────────────
--
-- Pela tabela das tres classes de falha (handoff v6, secao 4), isto e PRECO
-- ERRADO: a cobertura NAO cai, nenhuma pendencia nasce, o produto aparece na
-- tela com cara de certo. Nada denuncia. Mesmo padrao do emoji colado, que
-- fechava 100% de cobertura escondendo produto que sumia.
--
-- ── O que muda, e o que NAO muda ───────────────────────────────────────────
--
-- Muda SO quando o bloco de modelo tem uma cor solta para cada preco nu: ai o
-- pareamento e por ordem (n-esima cor com n-esimo preco), o que acerta os dois
-- layouts. Contagens diferentes continuam no vizinho mais proximo, byte a byte
-- como antes.
--
-- O formato "cor NA LINHA do preco" nao passa por aqui de jeito nenhum: linha de
-- preco que ja tem cor nao entra em `precos_nus`, entao aquele bloco nao tem
-- preco nu para parear.
--
-- Prova: `ferramentas/prova_calc_parse.sql`, fixture B, o bloco `iPhone 14 Pro`.
-- ─────────────────────────────────────────────────────────────────────────────────
