...
function createExternalCalcWorkerRuntime(options = {}) {
  ...
  return {
    async fetch(request) {
      ...
      const salesProductSource = createPostgresSalesProductSource({
        supabaseUrl: config.supabaseUrl,
        anonKey: config.anonKey,
        accessToken,
        fetchImpl
      });
      const salesProductService = createSalesProductService({ source: salesProductSource });
      const salesProductApi = createSalesProductApiV0({ service: salesProductService, authenticate: authContext });
