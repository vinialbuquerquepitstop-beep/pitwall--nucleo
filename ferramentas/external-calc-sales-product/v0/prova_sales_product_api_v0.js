...
const {createSalesProductApiV0,VARIANT_PATH,TRADE_IN_PATH,SIM_PATH}=require('./sales-product-api');
let seen=[];
const api=createSalesProductApiV0({authenticate:async()=>({subject:'seller'}),service:{async resolveVariant(x){seen.push(['v',x]);return {ok:true}},async estimateTradeIn(x){seen.push(['t',x]);return {ok:true}},async simulateSale(x){seen.push(['s',x]);return {ok:true}}}});
(async()=>{for(const p of [VARIANT_PATH,TRADE_IN_PATH,SIM_PATH])assert.strictEqual((await api.handle({method:'POST',path:p,body:'{}'})).status,200);assert.strictEqual(seen.every(x=>x[1].auth_user_id==='seller'),true);
const unauth=createSalesProductApiV0({authenticate:async()=>null,service:{}});assert.strictEqual((await unauth.handle({method:'POST',path:VARIANT_PATH,body:'{}'})).status,401);
