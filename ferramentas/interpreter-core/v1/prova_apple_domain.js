...
check('shorthand bare 16 128 usa primeiro trigger apenas com condicao e cor', () => {
  const result = run(
    'apple-shorthand-16-128-first-trigger-guard',
    'Seminovos\n16 128GB\nPreto\nR$ 3.900\nR$ 4.000'
  );
  assert.strictEqual(result.records.length, 1);
  assert.strictEqual(result.records[0].fields.model.id, 'iphone_16_128gb');
  ...
  assert.strictEqual(result.records[0].fields.condition, 'Seminovo');
  assert.strictEqual(result.records[0].fields.color, 'Preto');
  assert.strictEqual(result.records[0].fields.price, 3900);
  assert.ok(result.segments.some(segment =>
    (segment.context_events || []).some(event =>
      event.reason === 'scoped_context_expired_after_record_trigger' &&
      event.field === 'model'
...
check('shorthand bare 16 128 se abstem sem condicao herdada', () => {
  const result = run(
    'apple-shorthand-16-128-first-trigger-no-condition',
    '16 128GB\nPreto\nR$ 3.900'
  );
  assert.strictEqual(result.records.length, 0);
  assert.ok(result.ambiguities.some(ambiguity =>
